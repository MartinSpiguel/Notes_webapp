...
def login():
    if request.method == 'POST':
        ...
        if user:
            ...
            flash('Account with that email doesn\'t exist, sign in first', category='error')
...
def sign_up():
    if request.method == 'POST':
        ...
        if user:
            if user.email == email:
                flash('Email alredy registered, log in', category='error')
        elif len(email) < 5:
            ...
            flash('Password must have at list 8 characters', category='error')
